...
df <- list(
    ...
    physician = bind_rows(
        ...
        bind_rows(
            read_csv(
                "../tmp/healsl_rd1_adult_age_v2.csv",
                col_select = all_of(phy_cols_age)
            ),
            read_csv(
                "../tmp/healsl_rd1_child_age_v2.csv",
                col_select = all_of(phy_cols_age)
            ),
            read_csv(
                "../tmp/healsl_rd1_neo_age_v2.csv",
                col_select = all_of(phy_cols_age)
            ),
            read_csv(
                "../tmp/healsl_rd2_adult_age_v2.csv",
                col_select = all_of(phy_cols_age)
            ),
            read_csv(
                "../tmp/healsl_rd2_child_age_v2.csv",
                col_select = all_of(phy_cols_age)
            ),
            read_csv(
                "../tmp/healsl_rd2_neo_age_v2.csv",
                col_select = all_of(phy_cols_age)
            )
        ) %>% rename(
            age_value = age_value_cod,
            age_unit = age_unit_cod
        ),
        by = "rowid"
    ),
    model_gpt3 = bind_rows(
        read_csv(
            "../tmp/healsl_rd1_rapid_gpt3_v2b_first.csv",
            col_select = c("rowid", "cause1_icd10")
        ) %>% mutate(
            cause1_icd10 = str_sub(cause1_icd10, 1, 3),
            round = 1
        ),
        read_csv(
            "../tmp/healsl_rd2_rapid_gpt3_v2b_first.csv",
            col_select = c("rowid", "cause1_icd10")
        ) %>% mutate(
            cause1_icd10 = str_sub(cause1_icd10, 1, 3),
            round = 2
        )
    ) %>% rename(gpt3_icd10 = cause1_icd10),
    model_gpt4 = bind_rows(
        read_csv(
            "../tmp/healsl_rd1_rapid_gpt4_v2b_first.csv",
            col_select = c("rowid", "cause1_icd10")
        ) %>% mutate(
            cause1_icd10 = str_sub(cause1_icd10, 1, 3),
            round = 1
        ...
        read_csv(
            "../tmp/healsl_rd2_rapid_gpt4_v2b_first.csv",
            col_select = c("rowid", "cause1_icd10")
        ) %>% mutate(
            cause1_icd10 = str_sub(cause1_icd10, 1, 3),
            round = 2
        )
    ) %>% rename(gpt4_icd10 = cause1_icd10),
    model_gpt5 =
        read_csv(
            "../tmp/healsl_rd1to2_rapid_gpt5_v1.csv",
            col_select = c("rowid", "cause1_icd10", "round")
        ) %>% mutate(
            cause1_icd10 = str_sub(cause1_icd10, 1, 3),
            round = as.integer(substr(round, 3, 3))
        ) %>% rename(gpt5_icd10 = cause1_icd10),
...
### Generalize Age Range

Manually generalize age ranges for each age group:
...
# Get gpt3 coded records
out_cgpt3 <- out_coded %>% filter(
    !is.na(gpt3_icd10) & !is.null(gpt3_icd10)
)
...
# Get gpt3 coded records converted to cghr10
out_cgpt3_xcghr10 <- out_cgpt3 %>% filter(
    !is.na(gpt3_cghr10) & !is.null(gpt3_cghr10)
)
...
# Get gpt4 coded records
out_cgpt4 <- out_coded %>% filter(
    !is.na(gpt4_icd10) & !is.null(gpt4_icd10)
)
...
# Get gpt4 coded records converted to cghr10
out_cgpt4_xcghr10 <- out_cgpt4 %>% filter(
    !is.na(gpt4_cghr10) & !is.null(gpt4_cghr10)
)
...
# Get gpt5 coded records
out_cgpt5 <- out_coded %>% filter(
    !is.na(gpt5_icd10) & !is.null(gpt5_icd10)
)
out_cgpt5_agree <- out_coded_agree %>% filter(
    !is.na(gpt5_icd10) & !is.null(gpt5_icd10)
)
out_cgpt5_cghr10 <- out_coded_cghr10 %>% filter(
    !is.na(gpt5_icd10) & !is.null(gpt5_icd10)
)
out_cgpt5_cghr10_agree <- out_coded_cghr10_agree %>% filter(
    !is.na(gpt5_icd10) & !is.null(gpt5_icd10)
)

# Get gpt5 coded records converted to cghr10
out_cgpt5_xcghr10 <- out_cgpt5 %>% filter(
    !is.na(gpt5_cghr10) & !is.null(gpt5_cghr10)
)
out_cgpt5_xcghr10_agree <- out_cgpt5_agree %>% filter(
    !is.na(gpt5_cghr10) & !is.null(gpt5_cghr10)
)
out_cgpt5_xcghr10_cghr10 <- out_cgpt5_cghr10 %>% filter(
    !is.na(gpt5_cghr10) & !is.null(gpt5_cghr10)
)
out_cgpt5_xcghr10_cghr10_agree <- out_cgpt5_cghr10_agree %>% filter(
    !is.na(gpt5_cghr10) & !is.null(gpt5_cghr10)
)
...
cat(paste0(
    "ICD-10 Output\n-------------",
    "\nChatGPT-5 ICD-10 Coded (Phy. ICD-10 Coded): ", out_cgpt5 %>% nrow,
    "\nChatGPT-5 ICD-10 Coded (Agree, Phy. ICD-10 Coded): ", out_cgpt5_agree %>% nrow,
    "\nChatGPT-5 ICD-10 Coded (Phy. CGHR-10 Coded): ", out_cgpt5_cghr10 %>% nrow,
    "\nChatGPT-5 ICD-10 Coded (Agree, Phy. CGHR-10 Coded): ", out_cgpt5_cghr10_agree %>% nrow,
    "\n\nAfter Conversion to CGHR-10\n---------------------------",
    "\nChatGPT-5 CGHR-10 Coded (Phy. ICD-10 Coded): ", out_cgpt5_xcghr10 %>% nrow,
    "\nChatGPT-5 CGHR-10 Coded (Agree, Phy. ICD-10 Coded): ", out_cgpt5_xcghr10_agree %>% nrow,
    "\nChatGPT-5 CGHR-10 Coded (Phy. CGHR-10 Coded): ", out_cgpt5_xcghr10_cghr10 %>% nrow,
    "\nChatGPT-5 CGHR-10 Coded (Agree, Phy. CGHR-10 Coded): ", out_cgpt5_xcghr10_cghr10_agree %>% nrow
))
```
...
out_iva5_cghr10_agree <- out_who_cghr10_agree %>% filter(
    !is.na(interva5_wva2016) & !is.null(interva5_wva2016)
)
...
out_isva_cghr10_agree <- out_who_cghr10_agree %>% filter(
    !is.na(insilicova_wva2016) & !is.null(insilicova_wva2016)
)
...
```{r}
write_csv(out, "../data/healsl_rd1to2_cod_v2.csv")
```