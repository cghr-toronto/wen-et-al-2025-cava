...
Clean `age_range` labels to include only values and unit of measure.

```{r}
df <- raw_df %>% mutate(
    "age_range" = str_replace(
        age_range,
        "\\s*\\(.*\\)",
        ""
    ))
...
codplot <- coddata %>%
    ...
    labs(
        x = paste0("Physician Agreed Records (n=", nagree, ", 100%) for ", length(causes_col), " Causes of Death"),
        y = "PCCC (0=Low, 1=High)"
    ) +
    ylim(-0.3, 1.3) +
    coord_flip() +
