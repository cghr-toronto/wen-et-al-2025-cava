...
calc_by <- function(
    ...
    # Calc metric by detailed age range
    if (by_age_range) {
    ...
    # Calc metric by general age range
    if (by_age_range2) {
        ...
        for (a in c("adult", "child", "neo")) {
            for (ar in age_ranges) {
                ...
                # Filter for age range
                df_filter <- df %>% filter(
                    age_range2 == ar & is_agreed == TRUE & age == a
                )
                metric_name <- sprintf(
                    "%s %s Age2 Agree %s", name, str_to_title(a), ar
                )
...
df <- raw_df %>% mutate(
    ...
    "age_range2" = str_to_title(str_replace(
        age_range2,
        "\\s*\\(.*\\)",
        ""
    )),
    "age_range2" = if_else(
        !str_detect(age_range2, "Year|Years|Day|Days|month|Months|week|Weeks"),
        paste0(age_range2, " Years"),
        age_range2
    )
)
df
...
Calculate Partial Chance Corrected Concordance (PCCC) to evaluate individual performance for each model by age and physician coding stage.
