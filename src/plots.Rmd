...
# Standardize raw ages to age range + unit
df_raw <- df_raw %>%
    mutate(
        age_range_orig = age_range,
...
# Order raw age ranges as factor
df_raw <- df_raw %>%
    mutate(
        age_range = factor(
            age_range %>%
...
ggsave(
    "../manuscript/figures/png/fig-pccc-csmf.png",
    fig_pccc_csmf,
    width = 170,
    height = 100,
    units = "mm",
...
ggsave(
    "../manuscript/figures/png/fig-pccc.png",
    fig_pccc,
    width = 170,
    height = 75,
    units = "mm",
...
fig_pccc_cod_adult <- plot_perf_bar(
    df_long %>% filter(age == "adult"),
    calc_pccc,
    group = "cod_name",
    title = paste0("Adult Causes of Death, 12+ Years (n=", n_adult, ")"),
    text_scale = 0.7,
    x_hpos = -0.27
)
...
ggsave(
    "../manuscript/figures/png/fig-pccc-cod-adult.png",
    fig_pccc_cod_adult,
    width = 170, # min 170 mm full / 85 mm half
    height = 225, # max 225 mm
    units = "mm",
...
fig_pccc_cod_child <- plot_perf_bar(
    df_long %>% filter(age == "child"),
    calc_pccc,
    group = "cod_name",
    title = paste0("Child Causes of Death, 28 Days to 11 Years (n=", n_child, ")"),
    text_scale = 0.92,
    x_hpos = -0.43
)
...
ggsave(
    "../manuscript/figures/png/fig-pccc-cod-child.png",
    fig_pccc_cod_child,
    width = 170,
    height = 150,
    units = "mm",
...
fig_pccc_cod_neo <- plot_perf_bar(
    df_long %>% filter(age == "neo"),
    calc_pccc,
    group = "cod_name",
    title = paste0("Neonatal Causes of Death, <28 Days (n=", n_neo, ")"),
    x_title = "Partial Chance Corrected Concordance (PCCC)",
    text_scale = 0.92,
    x_hpos = -0.43
)
...
ggsave(
    "../manuscript/figures/png/fig-pccc-cod-neo.png",
    fig_pccc_cod_neo,
    width = 170,
    height = 100,
    units = "mm",
...
ggsave(
    "../manuscript/figures/png/fig-pccc-age.png",
    fig_pccc_age,
    width = 170,
    height = 100,
    units = "mm",
...
# Get total records
tab_all <- df_raw %>%
    filter(!is.na(physician_cghr10) & !is.null(physician_cghr10)) # remove uncoded cases
tab_all_n <- tab_all %>% nrow
...
tab_agegroup <- df %>%
    ...
    mutate(
        n_agree_perc = round(n / n_all * 100, 1) 
    )
...
tab_cod_n <- df %>%
    ...
    left_join(
        tab_all %>%
        group_by(age, physician_cghr10) %>%
        summarize(n_cod_all = n()) %>%
        ungroup,
        by = c("age", "physician_cghr10")
    ) %>%
    mutate(
        n_cod_agree_perc = round(n_cod / n_cod_all * 100 , 1)
    )
...
tab_cod <- tab_cod %>%
    mutate(
        ...
        `Age Group` = glue(
            "{age} (n={n}, {n_perc}%, {n_cod_uniq} CODs)\n- Female (n={n_female}, {n_female_perc}%)\n- Male (n={n_male}, {n_male_perc}%)\n- Agree ({n_agree_perc}% of {n_all})"
        ),
        `Cause of Death (COD)` = glue(
            "{str_to_title(physician_cghr10)}"
        ),
        `Agree` = glue("{n_cod_agree_perc}% of {n_cod_all}"),
...
tab_age_n <- df %>%
    ...
    left_join(
        tab_all %>%
        group_by(age, age_range) %>%
        summarize(n_age_all = n()) %>%
        ungroup,
        by = c("age", "age_range")
    ) %>%
    mutate(
        n_age_agree_perc = round(n_age / n_age_all * 100 , 1)
    )
...
tab_age <- tab_age %>%
    mutate(
        ...
        `Agree` = glue("{n_age_agree_perc}% of {n_age_all}"),
...
tab_age_format <- tab_age %>%
    ungroup %>%
    select(`Age Group`, `Age Range`, Agree, Female, Male, Total) %>%
    mutate(
        `Age Group` = str_replace_all(`Age Group`, "\\\\n|\\n", "<br>"),
