...
# Calc pccc
df_plot <- df_long %>%
    group_by(age_name, cod_name, model_name, .drop = F) %>%
    summarize( # calc pccc
        pccc = calc_pccc(cod, model_cghr10, N = unique(ncod)[1])
    ) %>%
    mutate( # add pccc to model name
        model_label = paste0(model_name, " (", round(pccc, 2), ")")
    ) %>%
    filter(age_name == "Adult")
...
df_ends <- df_plot %>%
    ...
    summarize(
        min_pccc = min(pccc),
        max_pccc = max(pccc),
        model_min = model_name[which.min(pccc)],
        model_max = model_name[which.max(pccc)]
    ) %>%
    arrange(max_pccc)

# Sort cod by max model pccc
df_plot <- df_plot %>%
    mutate(cod_name = factor(cod_name, levels = df_ends$cod_name))
df_ends <- df_ends %>%
    mutate(cod_name = factor(cod_name, levels = df_ends$cod_name))

# Filter out the mid models
df_mids <- df_plot %>%
    left_join(df_ends, by = "cod_name") %>%
    filter(model_name != model_max & model_name != model_min)

# Plot
ggplot(df_plot, aes(x = pccc, y = cod_name)) +
    geom_point( # mid points
        data = df_mids,
        color = "gray60",
        size = 1
    ) +
    geom_point (# min points
        data = df_ends,
        aes(x = min_pccc),
        size = 1.5
    ) +
    geom_point (# max points
        data = df_ends,
        aes(x = max_pccc),
        size = 1.5
    ) +
    ...
    geom_text( # min model text
        ...
        aes(
            x = min_pccc,
            y = cod_name,
            label = paste0(model_min, " (", round(min_pccc, 2), ")")
        ),
        hjust = 1.2,
        vjust = 0.5,
        size = 3
    ...
    geom_text( # max model text
        ...
        aes(
            x = max_pccc,
            y = cod_name,
            label = paste0(model_max, " (", round(max_pccc, 2), ")")
        ),
        hjust = -0.2,
        vjust = 0.5,
        size = 3
    ...
    labs(
        x = "Partial Chance Corrected Concordance (PCCC)",
        y = paste0(length(unique(df_plot$cod_name)), " Causes of Death for ", n_adult, " Adult Records")
    ) +
    theme_minimal() +
    theme(
