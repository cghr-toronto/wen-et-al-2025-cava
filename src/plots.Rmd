...
Display a plot of cases by age range.
...
eardata <- df %>%
    ...
    mutate(
        ...
        age_range_label = paste0(
            ...
            " ",
            age_range_unit,
            "\n(n=", count, ", ", round((count / nagree) * 100), "%)"
        )
    ) %>%
...
# Save the plot
earplot
ggsave("../manuscript/fig-data-agerange.pdf", plot = earplot, dpi = 300)
```

#### Cases by COD

Display a plot of cases by cause of death.
    ...
    mutate(
        cod_label = paste0(
            ...
            if_else(
                ((count / nagree) * 100) < 1,
                "<1",
                as.character(round((count / nagree) * 100))
            ), "%)"
        )
    ) %>%
    arrange(count) %>%
...
# Save the plot
ecodplot
ggsave("../manuscript/fig-data-cod.pdf", plot = ecodplot, dpi = 300)
```
