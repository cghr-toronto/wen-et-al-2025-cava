...
calc_by <- function(
    ...
    if (by_cod) {
        ...
        for (cod in causes) {
            ...
            # Filter data for cod
            df_filter <- df %>% filter(physician_cghr10 == cod & is_agreed == TRUE)
            metric_name <- paste0(name, " COD Agree ", cod)
    ...
    if (by_age_range) {
        ...
        for (ar in age_ranges) {
            ...
            df_filter <- df %>% filter(age_range == ar & is_agreed == TRUE)
            metric_name = paste0(name, " Age Agree ", ar)
...
causes_col <- out %>% select(starts_with("PCCC COD Agree ")) %>% names
...
for (cod_col in causes_col) {
    ...
    cod <- gsub("PCCC COD Agree ", "", cod_col)
...
Display a plot of performance for physician agreed records by age ranges.
...
arange_col <- out %>% select(starts_with("PCCC Age Agree ")) %>% names
...
for (ar_col in arange_col) {
    ...
    # Get cod without prefix
    ar <- gsub("PCCC Age Agree ", "", ar_col)
    
    # Calculate num and perc cases for age range
    narange <- df %>% filter(age_range == ar & is_agreed == TRUE) %>% nrow
    parange <- (narange / nagree) * 100
    parange_label <- if (round(parange) >= 1) round(parange) else "<1"
    
    # Break age range into newlines if more than 3 words
    if (str_count(ar, "\\s+") > 3) {
        arange_label <- str_replace(ar, "(\\S+\\s+\\S+\\s+\\S+) ", "\\1\n")
    } else {
        arange_label <- ar
    }
    
    # Add label for age ranges
    arange_remap[[ar]] <- paste0(arange_label, "\n(n=", narange, ", ", parange_label, "%)")
...
# Save the plot
arplot
ggsave("../manuscript/fig-perf-agerange.pdf", plot = arplot, dpi = 300)
```
