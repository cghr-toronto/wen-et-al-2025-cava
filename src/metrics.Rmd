...
Calculate Partial Chance Corrected Concordance (PCCC) to evaluate indivudal performance for each model.

```{r}
pccc <- list()

# Calc overall pccc
pccc$all <- calc_per_model(
...
for (age_group in c("adult", "child", "neo")) {
    ...
    metric_name = paste0("PCCC ", str_to_title(age_group))

    # Calc metric for age
    pccc[[age_group]] <- calc_per_model(
        actual = df_age %>%
...
Calculate Cause Specific Mortality Fraction (CSMF) Accuracy to evaluate population performance for each model.

```{r}
csmf_acc <- list()
```

#### Overall CSMF Accuracy

```{r}
csmf_acc <- calc_per_model(
    physicians,
    models,
    calc_csmf_acc,
    "CSMF Accuracy"
