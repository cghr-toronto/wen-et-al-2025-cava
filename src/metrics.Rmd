...
col_agree <- paste0(
    "Agreement\n(n=",
    nrow(df %>% filter(age == "adult" & is_agreed == TRUE)),
    ")"
)
col_recon <- paste0(
    "Reconciliation\n(n=",
    nrow(df %>% filter(age == "adult" & is_recon == TRUE)),
    ")"
)
col_adj <- paste0(
    "Adjudication\n(n=",
    nrow(df %>% filter(age == "adult" & is_adj == TRUE)),
    ")"
)

# Add count to col names
col_rename <- list()
col_rename[[col_overall]] <- "Overall"
col_rename[[col_agree]] <- "Agreement"
col_rename[[col_recon]] <- "Reconciliation"
col_rename[[col_adj]] <- "Adjudication"
...
out %>%
    select(
        ...
        `PCCC Adult Reconciliation`,
        `PCCC Adult Adjudication`
    ) %>%
    rename(
         Overall = `PCCC Adult`,
         Agreement = `PCCC Adult Agreement`,
         Reconciliation = `PCCC Adult Reconciliation`,
         Adjudication = `PCCC Adult Adjudication`
    ) %>%
    rename(!!!col_rename)
```
...
pdata <- out %>%
    ...
    mutate( # Add min, mid, and max model names to label
        ...
        "PCCC Mid" = if_else(is.na(`PCCC Min`) & is.na(`PCCC Max`), paste0(Model, " (", round(PCCC, 2), ")"), NA),
...
pdata %>%
    ...
    geom_text(
        ...
        color = "#3d3d3d",
        size = 2,
        position = position_nudge(y = -0.06, x = -0.3)
    ) +
    geom_text(
        aes(label = `PCCC Max`),
        color = "#3d3d3d",
