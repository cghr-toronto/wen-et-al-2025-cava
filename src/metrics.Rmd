...
Create column for final physician ICD-10 codes based on the following rule:
...
df$physician <- df$physician %>%
    mutate(physician_icd10 = coalesce(
        adj_icd,
        p1_recon_icd,
        p1_icd
    ))
...
Calculate PCCC for each model:

```{r}
```

Calculate CSMF Accuracy for each model:
