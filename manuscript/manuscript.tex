...
Figure \ref{fig-pccc-cod-adult} shows model performance by PCCC across 17 adult CODs excluding suicide, due to a low 3 (\textless 1\%) records. GPT-4 had the highest individual performance for 10 of 17 CODs (0.35 to 0.99 PCCC), GPT-3.5 for 5 CODs (0.43-0.94 PCCC), and InSilicoVA for 2 CODs (0.71 and 0.84 PCCC). InterVA-5 had the lowest performance for 8 of 17 CODs (0-0.79 PCCC), InSilicoVA for 6 CODs (0.01-0.41 PCCC), and GPT-3.5 for 2 CODs (0.38 and 0.53 PCCC). GPT-3.5/4 models improved over InSilicoVA/InterVA-5 the most for chronic respiratory diseases (0.74-0.94 PCCC difference), and the least for Malaria (0.09-0.17 PCCC difference). All models performed well for maternal conditions (0.79-0.99 PCCC), while unspecified infections, malaria, and ill-defined CODs had low performing models (\textless 0.5 PCCC). GPT-4 had large performance improvements compared to all other models for cancers (+0.25-0.36 PCCC), stroke (+0.27-0.45 PCCC), and diarrhoeal diseases (+0.37-0.51 PCCC), while GPT-3.5 had similarly large improvements for liver and alcohol related diseases (+0.27-0.52 PCCC).
...
Across 5 neonatal CODs (excluding congenital anomalies and other due to small sizes of 2 (\textless 1\%) and 5 (1\%) respectively), the highest PCCC was observed for GPT-4 for 3 of the 5 CODs (GPT-3.5 identical for prematurity and low birthweight), GPT-3.5 for 2 of the 5 CODs (InSilicoVA identical for stillbirth), and InSilicoVA for neonatal infections (Figure \ref{fig-pccc-cod-neo}). Stillbirth had small differences between minimum and maximum PCCC at 0.07. The other 4 CODs had large differences between minimum and maximum PCCC ranging from 0.46 to 0.64. Each neonatal COD had between 23 (5\%) to 172 (38\%) records, with stillbirth (n=172, 36\%), birth asphyxia and birth trauma (n=103, 22\%), and neonatal infections (n=99, 21\%), and prematurity and low birthweight (n=73, 15\%) making 94\% of all records, while the rest are in ill-defined.
...
\subsubsection{Performance for Particular Causes of Death}\label{discuss-adv-cod}
...
\section{Data Exploration}\label{app-data}
...
\caption{All records by age group and five-year age range.}\label{fig-data-agesex}
\end{figure}
...
\caption{All records by five-year age range.}\label{fig-data-agerange}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{fig-data-agerange-adult.pdf}
\caption{Adult records by five-year age range.}\label{fig-data-agerange-adult}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{fig-data-cod-adult.pdf}
\caption{Adult records by COD.}\label{fig-data-cod-adult}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{fig-data-agerange-child.pdf}
\caption{Child records by five-year age range.}\label{fig-data-agerange-child}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{fig-data-cod-child.pdf}
\caption{Child records by COD.}\label{fig-data-cod-child}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{fig-data-agerange-neo.pdf}
\caption{Neonatal records by five-year age range.}\label{fig-data-agerange-neo}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{fig-data-cod-neo.pdf}
\caption{Neonatal records by COD.}\label{fig-data-cod-neo}
\end{figure}
