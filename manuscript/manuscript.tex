...
The ChatGPT-3.5 and ChatGPT-4 Application Programming Interface (API) was accessed using Python version 3.11.4 and used to assign CODs for each record. ChatGPT-3.5 used the \verb|gpt-3.5-turbo| model, while ChatGPT-4 used the \verb|gpt-4-0613| mode. The parameter \verb|top_p| for ChatGPT-3.5 and ChatGPT-4, representing the sampling temperature, was set to 0.3 to consider only the top 30\% probability mass from the token results \cite{openaiOpenAIPlatformAPI2024}. Higher values closer to 1 may make the results more random, while lower values closer to 0 make the results more focused.
...
Of the 6939 records, ChatGPT-4 had the best performance overall followed by ChatGPT-3.5, InSilicoVA, and InterVA-5 at 0.71, 0.64, 0.44, and 0.43 PCCC respectively. CSMF Accuracies followed the same pattern except for InterVA-5 and InSilicoVA, where InterVA-5 had a higher CSMF Accuracy at 0.78 compared to InSilicoVA at 0.74. Figure \ref{fig-perf-agegroup} shows the PCCC performance across age groups with reference to the CSMF Accuracy. ChatGPT-4 also had the best performance across all three age groups, while ChatGPT-3.5 and InSilicoVA had similar performance (within 0.05 PCCC) for child and neonatal records respectively. ChatGPT-3.5 did worse than InSilicoVA for neonatal records at 0.51 PCCC whereas it had performances close to ChatGPT-4 for the adult and child age groups.

\begin{table}[h]
\caption{Overall model performance for physician agreed records (n=6939)}\label{tab-perf-overall}%
\begin{tabular}{@{}llll@{}}
\toprule
Model & PCCC (0=Low, 1=High)  & CSMF Accuracy (0=Low, 1=High)\\
\midrule
ChatGPT-4 & 0.71 & 0.89\\
ChatGPT-3.5 & 0.64 & 0.86\\
InSilicoVA & 0.44 & 0.74\\
InterVA-5 & 0.43 & 0.78\\
\botrule
\end{tabular}
...
\caption{Model performance by age group. ChatGPT-3.5, ChatGPT-4, InterVA-5, and InSilicoVA model performance for assigning CODs in the adult (12 to 69 Years), child (28 days to 11 years), and neonatal (under 28 days) age groups. CSMF indicates CSMF accuracy.}\label{fig-perf-agegroup}
\end{figure}
...
\caption{Model performance for adult records by age range. ChatGPT-3.5, ChatGPT-4, InterVA-5, and InSilicoVA model performance for assigning CODs in the adult age group (12 to 69 Years) by five-year age ranges. PCCC values are sorted by the highest to lowest PCCC for each age range. Only models with PCCC values within .05 of the highest and lowest PCCC are shown.}\label{fig-perf-agerange-adult}
\end{figure}

\begin{figure}[h!]
\centering
\includegraphics[width=0.7\textwidth]{fig-perf-cod-adult.pdf}
\caption{Model performance for adult records by COD. ChatGPT-3.5, ChatGPT-4, InterVA-5, and InSilicoVA model performance for assigning CODs in the adult age group (12 to 69 Years) by COD. PCCC values are sorted by the highest to lowest PCCC for each age range. Only models with PCCC values within .05 of the highest and lowest PCCC are shown.}\label{fig-perf-cod-adult}
\end{figure}
...
\caption{Model performance for child records by age range. ChatGPT-3.5, ChatGPT-4, InterVA-5, and InSilicoVA model performance for assigning CODs in the child age group (28 days to 11 years) by five-year age ranges. PCCC values are sorted by the highest to lowest PCCC for each age range. Only models with PCCC values within .05 of the highest and lowest PCCC are shown.}\label{fig-perf-agerange-child}
\end{figure}

\begin{figure}[h!]
\centering
\includegraphics[width=0.7\textwidth]{fig-perf-cod-child.pdf}
\caption{Model performance for child records by COD. ChatGPT-3.5, ChatGPT-4, InterVA-5, and InSilicoVA model performance for assigning CODs in the child age group (28 days to 11 years) by COD. PCCC values are sorted by the highest to lowest PCCC for each age range. Only models with PCCC values within .05 of the highest and lowest PCCC are shown.}\label{fig-perf-cod-child}
\end{figure}
...
\caption{Model performance for neonatal records by age range. ChatGPT-3.5, ChatGPT-4, InterVA-5, and InSilicoVA model performance for assigning CODs in the neonatal age group (under 28 days) by five-year age ranges. PCCC values are sorted by the highest to lowest PCCC for each age range. Only models with PCCC values within .05 of the highest and lowest PCCC are shown.}\label{fig-perf-agerange-neo}
\end{figure}

\begin{figure}[h!]
\centering
\includegraphics[width=0.7\textwidth]{fig-perf-cod-neo.pdf}
\caption{Model performance for neonatal records by COD. ChatGPT-3.5, ChatGPT-4, InterVA-5, and InSilicoVA model performance for assigning CODs in the neonatal age group (under 28 years) by COD. PCCC values are sorted by the highest to lowest PCCC for each age range. Only models with PCCC values within .05 of the highest and lowest PCCC are shown.}\label{fig-perf-cod-neo}
\end{figure}
