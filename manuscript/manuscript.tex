...
Of the 6939 records, GPT-4 had the best performance overall followed by GPT-3.5, InSilicoVA, and InterVA-5 at 0.71, 0.64, 0.44, and 0.43 PCCC respectively. CSMF Accuracies followed the same pattern except for InterVA-5 and InSilicoVA, where InterVA-5 had a higher CSMF Accuracy at 0.78 compared to InSilicoVA at 0.74. Figure \ref{fig-perf-agegroup} shows the PCCC performance across age groups with reference to the CSMF Accuracy. GPT-4 also had the best performance across all three age groups, while GPT-3.5 and InSilicoVA had similar performance (within 0.05 PCCC) for child and neonatal records respectively. GPT-3.5 did worse than InSilicoVA for neonatal records at 0.51 PCCC whereas it had performances close to GPT-4 for the adult and child age groups.

\begin{table}[h]
\caption{Overall model performance for physician agreed records (n=6939)}\label{tab-perf-overall}%
\begin{tabular}{@{}llll@{}}
\toprule
Model & PCCC\footnotemark[1]  & CSMF Accuracy\footnotemark[2] & PCCC Improvement from Model Below\\
\midrule
GPT-4 & 0.71 & 0.89 & +0.07\\
GPT-3.5 & 0.64 & 0.86 & +0.20\\
InSilicoVA & 0.44 & 0.74 & +0.01\\
InterVA-5 & 0.43 & 0.78 & -\\
\botrule
\end{tabular}
\footnotetext{Note: Models sorted from highest to lowest PCCC.}
\footnotetext[1]{Partial Chance Corrected Concordance (PCCC). Values of 0 (low) to 1 (high).}
\footnotetext[2]{Cause Specific Mortality Fraction (CSMF) Accuracy. Values of 0 (low) to 1 (high).}
\end{table}
