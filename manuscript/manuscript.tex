...
For GPT-3.5 and GPT-4, 6939 text prompts were generated for each physician agreed record as input to instruct the models to assign CODs based on the open narratives. Prompts were generated with a template involving replacing the placeholder \verb|<narrative>| using the free-text open narratives:
...
The GPT-3.5 and GPT-4 Application Programming Interface (API) was accessed using Python version 3.11.4 and used to assign CODs for each record. GPT-3.5 used the \verb|gpt-3.5-turbo| model, while GPT-4 used the \verb|gpt-4-0613| mode. The parameter \verb|top_p| for GPT-3.5 and GPT-4, representing the sampling temperature, was set to 0.3 to consider only the top 30\% probability mass from the token results \cite{openaiOpenAIPlatformAPI2024}. Higher values closer to 1 may make the results more random, while lower values closer to 0 make the results more focused.
...
Of the 6939 input records, GPT-3.5, GPT-4, InterVA-5, and InSilicoVA were able to assign CODs for 6929, 6909, 6805, 6729 records respectively. All 6805 InterVA-5 and 6729 InSilicoVA records with WHO VA 2016 v1.5 output codes \cite{worldhealthorganizationVerbalAutopsyStandards2016} were converted into ICD-10 codes. After all model outputs used ICD-10 codes, they were further converted to CGHR-10 codes. The 6929 GPT-3.5 and 6909 GPT-4 output records with ICD-10 codes were converted into 6892 and 6856 records with CGHR-10 codes, where 37 and 53 records did not have matching CGHR-10 codes respectively. All 6805 InterVA-5 and 6729 InSilicoVA records with ICD-10 codes were converted into records with CGHR-10 codes.
...
Of the 6939 records, GPT-4 had the highest PCCC followed by GPT-3.5, InSilicoVA, and InterVA-5 at 0.71, 0.64, 0.44, and 0.43 PCCC respectively (Table \ref{tab-perf-overall}). GPT-3.5 and GPT-4 had improvements ranging from 0.20-0.28 PCCC over InSilicoVA and InterVA-5, while GPT-4 slightly improved over GPT-3.5 by 0.07 PCCC. CSMF accuracies were above 0.7 for all models at 0.89, 0.86, 0.74, and 0.78 for GPT-4, GPT-3.5, InSilicoVA, and InterVA-5 respectively. Figure \ref{fig-perf-agegroup} shows the PCCC performance across age groups with reference to the CSMF Accuracy. GPT-4 had the highest PCCC across all three age groups at 0.71, 0.73, and 0.61 for adult, child, and neonatal age groups. GPT-3.5 had the next highest PCCC for the adult and child age groups at 0.62 and 0.69 PCCC respectively, performing slightly worse than GPT-4 by 0.09 and 0.04 PCCC. InSilicoVA had the second highest PCCC for the neonatal age group at 0.56, which was 0.05 PCCC worse than GPT-4. Both InterVA-5 and InSilicoVA had the lowest PCCC for the adult and child age groups at 0.41 and 0.45 PCCC respectively, while InterVA-5 and GPT-3.5 had the lowest PCCC for the neonatal age group at 0.49 and 0.51 respectively. The PCCC for models had larger differences between the minimum and maximum PCCC for the adult and child age groups at 0.3 and 0.28 respectively than the neonatal age group at 0.12.

\begin{table}[h]
\caption{Overall model performance for physician agreed records (n=6939)}\label{tab-perf-overall}%
\begin{tabular}{@{}llll@{}}
\toprule
Model & PCCC  & CSMF Accuracy & PCCC Improvement from Model Below\\
\midrule
GPT-4 & 0.71 & 0.89 & $\uparrow$ 0.07\\
GPT-3.5 & 0.64 & 0.86 & $\uparrow$ 0.20\\
InSilicoVA & 0.44 & 0.74 & $\uparrow$ 0.01\\
InterVA-5 & 0.43 & 0.78 & -\\
\botrule
\end{tabular}
\footnotetext{Note: Models sorted from highest to lowest PCCC.}
\end{table}

\begin{figure}[h!]
\centering
\includegraphics[width=0.7\textwidth]{fig-perf-agegroup.pdf}
\caption{Model performance by age group. GPT-3.5, GPT-4, InterVA-5, and InSilicoVA model performance for assigning CODs in the adult (12 to 69 Years), child (28 days to 11 years), and neonatal (under 28 days) age groups. CSMF indicates CSMF accuracy.}\label{fig-perf-agegroup}
\end{figure}

\subsection{Performance for Adult Records (12 Years or Older)}\label{results-adult}

Across the adult age ranges, GPT-4 had the highest PCCC values at or above 0.65, followed by GPT-3.5, while InterVA-5 and InSilicoVA had the lowest PCCC values at or below 0.47. GPT-4 PCCC was at or above 0.7 for young to middle age ranges between 10 to 49 years, and below 0.7 for older age ranges between 50 to 69 years (Figure \ref{fig-perf-agerange-adult}). Each age range had between 135 (4\%) to 575 (15\%) records. Figure \ref{fig-perf-cod-adult} shows the highest and lowest performing models across adult CODs. The highest PCCC values were GPT-4 between 0.53 to 1 for 15 of 18 CODs (GPT-3.5 within 0.05 for 7 of the 15 CODs), InSilicoVA for road and transport injuries at 0.84 (InterVA-5 close at 0.81), GPT-3.5 for ill-defined at 0.57,and InterVA-5 for ischemic heart disease at 0.61 (GPT-4 close at 0.57). The lowest PCCC values were InterVA-5 between 0 to 0.79 for 11 of 18 CODs (InSilicoVA within 0.05 for 3 of the 11 CODs), InSilicoVA between 0.01 to 0.41 for 6 of 18 CODs.

\begin{figure}[h!]
\centering
\includegraphics[width=0.7\textwidth]{fig-perf-agerange-adult.pdf}
\caption{Model performance for adult records by age range. GPT-3.5, GPT-4, InterVA-5, and InSilicoVA model performance for assigning CODs in the adult age group (12 to 69 Years) by five-year age ranges. PCCC values are sorted by the highest to lowest PCCC for each age range. Only models with PCCC values within .05 of the highest and lowest PCCC are shown.}\label{fig-perf-agerange-adult}
\end{figure}

\begin{figure}[h!]
\centering
\includegraphics[width=0.7\textwidth]{fig-perf-cod-adult.pdf}
\caption{Model performance for adult records by COD. GPT-3.5, GPT-4, InterVA-5, and InSilicoVA model performance for assigning CODs in the adult age group (12 to 69 Years) by COD. PCCC values are sorted by the highest to lowest PCCC for each age range. Only models with PCCC values within .05 of the highest and lowest PCCC are shown.}\label{fig-perf-cod-adult}
\end{figure}
...
\caption{Model performance for child records by age range. GPT-3.5, GPT-4, InterVA-5, and InSilicoVA model performance for assigning CODs in the child age group (28 days to 11 years) by five-year age ranges. PCCC values are sorted by the highest to lowest PCCC for each age range. Only models with PCCC values within .05 of the highest and lowest PCCC are shown.}\label{fig-perf-agerange-child}
\end{figure}

\begin{figure}[h!]
\centering
\includegraphics[width=0.7\textwidth]{fig-perf-cod-child.pdf}
\caption{Model performance for child records by COD. GPT-3.5, GPT-4, InterVA-5, and InSilicoVA model performance for assigning CODs in the child age group (28 days to 11 years) by COD. PCCC values are sorted by the highest to lowest PCCC for each age range. Only models with PCCC values within .05 of the highest and lowest PCCC are shown.}\label{fig-perf-cod-child}
\end{figure}
...
\caption{Model performance for neonatal records by age range. GPT-3.5, GPT-4, InterVA-5, and InSilicoVA model performance for assigning CODs in the neonatal age group (under 28 days) by five-year age ranges. PCCC values are sorted by the highest to lowest PCCC for each age range. Only models with PCCC values within .05 of the highest and lowest PCCC are shown.}\label{fig-perf-agerange-neo}
\end{figure}

\begin{figure}[h!]
\centering
\includegraphics[width=0.7\textwidth]{fig-perf-cod-neo.pdf}
\caption{Model performance for neonatal records by COD. GPT-3.5, GPT-4, InterVA-5, and InSilicoVA model performance for assigning CODs in the neonatal age group (under 28 years) by COD. PCCC values are sorted by the highest to lowest PCCC for each age range. Only models with PCCC values within .05 of the highest and lowest PCCC are shown.}\label{fig-perf-cod-neo}
\end{figure}
...
Did not explore rerun and response consistency.
